...
# 2. Run a batch (from src/, so the agent1 package imports):
python -m agent1.orchestrator \
...
2. **GPU not right-sized.** `fold_app/modal_app.py` uses H100; the 0.2B Fast model
   likely fits a cheaper GPU (A10G / L4). Test before scaling batches.
